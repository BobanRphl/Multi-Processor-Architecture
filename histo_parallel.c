...
int main(int argc, char* argv[]) {
   ...
   ******************************************************************* */
   ...
   if (my_rank == 0) {
	   ...
	   process_count = strtol(argv[4], NULL, 10);
	   ...
	   for (int i=1; i<=process_count; i++) {
		   ...
		   MPI_Send(&min_meas,1,MPI_FLOAT,i,0,MPI_COMM_WORLD);
		   MPI_Send(&max_meas,1,MPI_FLOAT,i,0,MPI_COMM_WORLD);
		   MPI_Send(&data_count,1,MPI_INT,i,0,MPI_COMM_WORLD);
		   MPI_Send(&data,1,MPI_INT,i,0,MPI_COMM_WORLD);
	   }

	   for (int i=1; i<=process_count; i++) {
		   MPI_Recv(&bin_count,1,MPI_INT,i,1,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
		   MPI_Recv(&bin_counts,1,MPI_INT,i,1,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
		   MPI_Recv(&bin_maxes,1,MPI_FLOAT,i,1,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
		   MPI_Recv(&min_meas,1,MPI_FLOAT,i,1,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
		   MPI_Recv(&data,1,MPI_FLOAT,i,1,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
		   MPI_Recv(&data_count,1,MPI_INT,i,1,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
	   }
...
#  endif
	 ...
	 MPI_Send(&bin_count,1,MPI_INT,0,1,MPI_COMM_WORLD);
	 MPI_Send(&bin_counts,1,MPI_INT,0,1,MPI_COMM_WORLD);
	 MPI_Send(&bin_maxes,1,MPI_FLOAT,0,1,MPI_COMM_WORLD);
	 MPI_Send(&min_meas,1,MPI_FLOAT,0,1,MPI_COMM_WORLD);
	 MPI_Send(&data,1,MPI_FLOAT,0,1,MPI_COMM_WORLD);
	 MPI_Send(&data_count,1,MPI_INT,0,1,MPI_COMM_WORLD);
   }
}  /* main */
